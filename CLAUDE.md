...
A group is only emitted when `buildResourceV2`/`buildResourceV3` finds a **create** operation plus **read or delete** — otherwise it returns `ok == false` and the group is dropped. Silent unless it has a create operation but no read or delete: that near-miss is recorded in `DiscoveryResult.Warnings` and logged once from `GetSchema` (`runtime.Build`) via the provider logger, in both library and parameterized modes. A `ResourceOverride` key (other than `"*"`) that matches no path group is a discovery error. Update is optional and is only read off the item path (`PUT` preferred over `PATCH`); `PUT`/`PATCH` on the collection path and `POST` on the item path are ignored, so body-ID updates need an override. A resource with `UpdatePath == ""` no-ops on update rather than erroring.
